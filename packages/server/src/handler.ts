...
import { matchServerRoutes, type RouteMatch } from "./route-matching";
import { createStaticHandlerDataRoutes, createRoutes } from "./routes";
import { json, isRedirectResponse, isCatchResponse } from "./responses";
import { createServerHandoffString } from "./server-handoff";
...
async function handleDataRequest({
  ...
  try {
    ...
    if (serverMode == "development" && error instanceof Error) {
      return errorBoundaryError(error, 500, serverMode);
    }
...
async function handleDocumentRequest({
  ...
  if (matches && isActionRequest(request)) {
    ...
    try {
      ...
      appState.error = await serializeError(error, serverMode);
...
async function errorBoundaryError(
  error: Error,
  status: number,
  serverMode: string
) {
  return json(await serializeError(error, serverMode), {
    status,
    headers: {
      "X-Remix-Error": "yes",
    },
...
function returnLastResortErrorResponse(error: any, serverMode: string) {
  if (serverMode != "test") {
    console.error(error);
  }
